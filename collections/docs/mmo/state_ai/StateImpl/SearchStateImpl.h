...
namespace StateAI
{
    ...
    class SummonSearchState : public SearchStateNormal
    {
    public:
        SummonSearchState(MachineType *machine) : SearchStateNormal(machine)
        {
        }

        void Execute(BaseType::EntityType *entity);

        DECL_LIFE_POLICY_FUNC(SummonSearchState, AIState);
    };
}
