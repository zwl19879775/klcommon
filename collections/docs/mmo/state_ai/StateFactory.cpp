...
#include "StateImpl/SummonStateImpl.h"
#include "StateImpl/QuickMoveState.h"
...
#define BIND(className, state, impl) \
	...
	StateFactory::StateFactory()
	{
        ...
        BIND(QuickMoveState, ST_QUICKMOVE, QMS_NORMAL);
        ...
        // summon state implementation
        BIND(SummonPeaceState  , ST_PEACE , PSI_SUMMON);
        BIND(SummonSearchState , ST_SEARCH, SSI_SUMMON_SEARCH_GLADIATOR);
        BIND(SummonDeadState   , ST_DEAD , DSI_SUMMON);
	}

	bool StateFactory::Add(long state_type, long state_impl, CreateAIStateT create, DestroyAIStateT destroy)
	{
		LogInfo(AI_MODULE, "state_name=%s, state_type=%d, state_impl=%d",
			BaseAI::State2Str(state_type), state_type, state_impl);
