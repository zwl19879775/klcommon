...
namespace StateAI
{
    ...
    struct QuickMoveEvent : public AIEvent
    {
        QuickMoveEvent (long destX, long destY, float speed) : AIEvent (ET_QUICKMOVE),
            m_destX (destX), m_destY (destY), m_speed (speed)
        {
        }
        long m_destX, m_destY;
        float m_speed;
    };
    ...
    typedef DefaultEvent<ET_QUICKMOVESTOP> QuickMoveStopEvent;
}
